...
//
// An interesting quirk of RemoveTarget is that Subject denotes a configuration
// object that, if the removed block is valid, should no longer exist in
// configuration. This "last known address" is used to locate and delete the
// appropriate state objects, or, in the case in which the user has forgotten
...
type RemoveTarget struct {
	...
	// Subject is the address of the resource or module in configuration.
	Subject ConfigMoveable
}

func (t *RemoveTarget) ObjectKind() RemoveTargetKind {
	return removeTargetKind(t.Subject)
}

func (t *RemoveTarget) String() string {
	if t.ObjectKind() == RemoveTargetModule {
		return t.Subject.(Module).String()
	} else if t.ObjectKind() == RemoveTargetResource {
		return t.Subject.(ConfigResource).String()
	}
